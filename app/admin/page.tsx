...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      // Quando for lançar o site de verdade, tire as duas barras (//) da linha abaixo para travar a tela!
  ...
  const buscarAgendamentos = async () => {
    const { data, error } = await supabase
      ...
      .order('data', { ascending: true }) // Ordena do dia mais próximo para o mais longe
      .order('hora', { ascending: true }); // Ordena da manhã pra noite
  ...
  // Função poderosa: O administrador cancela o horário e ele volta a ficar livre no site
      ...
      <div className="bg-zinc-900/30 rounded-3xl border border-zinc-800/60 p-6 md:p-8 shadow-2xl">
                ...
                <tr key={agendamento.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-all">
                  ...
                  <td className="p-4 text-zinc-300">
                    <div className="flex items-center gap-2 text-amber-500 font-bold">
                      <Calendar size={16} /> {formatarData(agendamento.data)}
                      <Clock size={16} className="ml-2" /> {agendamento.hora}
                    </div>
                  </td>
                  <td className="p-4">
        ...
        {agendamentos.length === 0 && (
          <div className="text-center py-12 text-zinc-500 font-bold">
            Sua agenda está livre! Nenhum agendamento confirmado no momento.
          </div>
        )}
      </div>
    </div>
